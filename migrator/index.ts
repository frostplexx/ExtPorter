...
function logMemoryUsage(context: string): void {
  ...
  if (isMonitoringEnabled) {
    logger.info(
      null,
      `Memory usage [${context}]: ${formatMemoryUsage(memUsage)}`,
    );
  } else {
    logger.debug(
      null,
...
function forceGarbageCollection(): void {
  ...
  if (global.gc) {
    ...
    if (isMonitoringEnabled) {
      ...
      logger.info(
        null,
        `Forced garbage collection completed, freed ${freedMB}MB`,
      );
    } else {
...
async function main() {
  ...
  logger.info(
    null,
    `Found ${extensions.length} extensions in ${globals.extensionsPath}`,
  );
  logger.info(
    null,
  ...
  for (
    ...
    for (let i = batchStart; i < batchEnd; i++) {
      ...
      if (!checkMemoryThreshold()) {
        logger.error(
          null,
          "Memory usage too high, stopping migration to prevent crash",
        );
        break;
      }
      ...
      if (migrationSuccessful) {
        ...
        try {
          ...
          await MigrationWriter.shared.writeExtensionSync(
            extension,
            outputPath,
          );
          ...
          try {
            ...
            logger.error(
              extension,
              `Failed to insert migrated extension to database`,
              {
                error:

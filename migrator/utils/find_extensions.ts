 ...
 */
export function find_extensions(
  ext_path: string,
  includes_mv3: boolean = false,
): Extension[] {
    ...
    try {
      ...
      if (json["manifest_version"] == 2 || includes_mv3) {
        ...
        if (!id) {
          logger.error(
            undefined,
            "Error getting extension id while searching",
            {
  ...
  for (const locale of localeOptions) {
    ...
    if (existsSync(messagesPath)) {
      ...
      try {
        ...
        logger.warn(
          null,
          `Failed to parse localization file: ${messagesPath}`,
          { error: error, messages_path: messagesPath },
        );
      } finally {
        // Ensure file descriptor is always closed
        if (messagesMMapFile) {
...
function findExtensionsRecursively(
  dirPath: string,
  includes_mv3: boolean,
): Extension[] {
  const extensions: Extension[] = [];
  ...
  try {
    ...
    logger.error(
      null,
      `Failed to read directory during recursive search: ${dirPath}`,
      { error: error, dir_path: dirPath },
    );
  }

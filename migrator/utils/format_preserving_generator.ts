...
export class FormatPreservingGenerator {
    ...
    private static getCodeContext(
        ...
        for (let i = startAfter; i < Math.min(lines.length, lineIndex + maxContextLines + 1); i++) {
            const line = lines[i]?.trim();
            if (line && !line.startsWith('//') && !line.startsWith('/*')) {
     ...
     */
    private static findNearestFunction(lines: string[], lineIndex: number): string | null {
     ...
     */
    private static findNearestVariable(lines: string[], lineIndex: number): string | null {
     ...
     */
    private static injectComments(generatedCode: string, comments: CommentInfo[]): string {
     ...
     */
    private static formatComment(comment: CommentInfo, overrideIndentation?: string): string[] {
    ...
    private static shouldCommentBeIncludedAtLine(
        ...
        if (comment.context.nearestFunction && codeLine.includes(comment.context.nearestFunction)) {
            return position === 'before';
        }

        if (comment.context.nearestVariable && codeLine.includes(comment.context.nearestVariable)) {
            return position === 'before';
        }
     ...
     */
    private static codePatternSimilarity(pattern1: string, pattern2: string): number {
    ...
    public static generateWithPreservedFormatting(
        ...
        const comments = preserveComments ? this.extractComments(originalSource) : [];

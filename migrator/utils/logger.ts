...
const currentLogLevel =
  LOG_LEVELS[ENV_LOG_LEVEL.toLowerCase()] ?? LOG_LEVELS["info"];
...
async function flushLogBatch() {
  ...
  for (const chunk of chunks) {
    try {
      ...
      console.error(
        `Failed to flush log chunk (${chunk.length} logs) to database:`,
        error,
      );
...
export const logger = {
  ...
  error: (extension: Extension | null = null, message: string, meta?: any) => {
    ...
    if (shouldLog(LogLevel.ERROR)) {
      console.error(
        `${getColoredLabel(LogLevel.ERROR)} ${message}`,
        meta || "",
      );
  ...
  debug: (extension: Extension | null = null, message: string, meta?: any) => {
    ...
    if (shouldLog(LogLevel.DEBUG)) {
      console.debug(
        `${getColoredLabel(LogLevel.DEBUG)} ${message}`,
        meta || "",
      );

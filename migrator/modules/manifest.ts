...
export class MigrateManifest implements MigrationModule {
        ...
        if (typeof existingCSP === "string") {
            ...
            if (existingCSP["extension_pages"]) {
                try {
                    ...
                    logger.warn(extension, "Invalid extension_pages CSP found, using MV3-compliant default", {
    ...
    private static validateAndSanitizeCSP(csp: string): string {
        ...
        logger.debug(null, "Sanitizing CSP for MV3 compliance", { originalCSP: sanitizedCSP });
        ...
        const hasValidStructure = /(?:script-src|style-src|img-src|object-src|default-src|connect-src|font-src|media-src|worker-src|frame-src|manifest-src|base-uri|form-action|frame-ancestors|plugin-types|sandbox|report-uri|report-to)\s+[^;]*;?/.test(sanitizedCSP);
        if (!hasValidStructure) {
            logger.warn(null, "CSP has no valid directive structure after sanitization, using safe default");
        ...
        const hasCriticalViolation = criticalViolations.some(pattern => pattern.test(sanitizedCSP));
        if (hasCriticalViolation) {
            logger.warn(null, "CSP still contains critical MV3 violations after sanitization, using safe default");
            return "script-src 'self'; object-src 'self';";
        }
    ...
    private static isMV3CompliantCSP(csp: string): boolean {
        ...
        const hasValidDirective = /(?:script-src|style-src|img-src|object-src|default-src|connect-src|font-src|media-src|worker-src|frame-src|manifest-src|base-uri|form-action|frame-ancestors|plugin-types|sandbox|report-uri|report-to)\s+[^;]*;?/.test(csp);
    ...
    private static sanitizeHashDirectives(csp: string): string {
        ...
        if (nonceMatches) {
            logger.debug(null, `Removing ${nonceMatches.length} nonce directive(s) for MV3 compliance`, {

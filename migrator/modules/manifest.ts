...
export class MigrateManifest implements MigrationModule {
  ...
  public static migrate(extension: Extension): Extension | MigrationError {
    try {
      ...
      extension.mv3_extension_id =
        MigrateManifest.generateMV3ExtensionId(extension);
      ...
      const resources = extension.manifest[
  ...
  private static generateMV3ExtensionId(extension: Extension): string {
    ...
    return crypto
      ...
      .substring(0, 32)
      .replace(/./g, (c: any) =>
        String.fromCharCode(97 + (parseInt(c, 16) % 26)),
      );
  }
  ...
  private static migrateContentSecurityPolicy(extension: Extension): void {
    ...
    if (typeof existingCSP === "string") {
      ...
      try {
        const validatedCSP =
          MigrateManifest.validateAndSanitizeCSP(existingCSP);
        ...
        logger.warn(
          extension,
          "Invalid CSP found, using default MV3-compliant CSP",
          {
            originalCSP: existingCSP,
            error: error instanceof Error ? error.message : String(error),
            reason:
              "CSP validation failed - likely contains MV3-incompatible directives",
          },
        );
        extension.manifest["content_security_policy"] = {
      ...
      if (existingCSP["extension_pages"]) {
        try {
          existingCSP["extension_pages"] =
            MigrateManifest.validateAndSanitizeCSP(
              existingCSP["extension_pages"],
            );
  ...
  private static validateAndSanitizeCSP(csp: string): string {
    ...
    if (!MigrateManifest.isMV3CompliantCSP(normalizedCSP)) {
      logger.warn(
        null,
        "CSP contains MV3-incompatible directives, sanitizing",
        {
    ...
    if (!sanitizedCSP.includes("script-src")) {
      ...
      sanitizedCSP = sanitizedCSP.replace(
        /script-src([^;]+);?/,
        "script-src 'self' $1;",
      );
    }
  ...
  private static sanitizeHashDirectives(csp: string): string {
    ...
    if (matches) {
      logger.debug(
        null,
        `Removing ${matches.length} hash directive(s) for MV3 compliance`,
        {

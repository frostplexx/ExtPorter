 ...
 */
export const migrateManifest = async (
    extension: Extension
): Promise<Extension | MigrationError> => {
    try {
        ...
        const { apiPermissions, hostPermissions } = splitPermissions(extension);
        ...
        addDeclarativeNetRequest(extension);

        // Migrate web_accessible_resources
        migrateWebAccessibleResources(extension);

        // Migrate browser_action and page_action to action
        migrateActions(extension);

        // Migrate background scripts to service worker
        migrateBackground(extension);
 ...
 */
function splitPermissions(extension: Extension) {
 ...
 */
function addDeclarativeNetRequest(extension: Extension): void {
 ...
 */
function migrateWebAccessibleResources(extension: Extension): void {
 ...
 */
function migrateActions(extension: Extension): void {
 ...
 */
function migrateBackground(extension: Extension): void {
...
/**
 * Backward compatibility class that wraps the migrateManifest function
 */
export class MigrateManifest implements MigrationModule {
    public static async migrate(extension: Extension): Promise<Extension | MigrationError> {
        return migrateManifest(extension);
    }
}

...
export class ResourceDownloader extends MigrationModule {
  ...
  private processExtension(extension: Extension): Extension {
    ...
    const downloadResults = this.downloadResources(
      extensionCopy,
      remoteResources,
    );
    const updatedExtension = this.updateReferencesToLocal(
      extensionCopy,
      downloadResults,
    );
  ...
  private extractUrlsFromObject(obj: any, resources: Set<string>): void {
    ...
    if (typeof obj === "string") {
      ...
      Object.values(obj).forEach((value) =>
        this.extractUrlsFromObject(value, resources),
      );
    }
  }
    ...
    for (const resource of resources) {
      try {
        ...
        if (result.success && result.localPath) {
          ...
          this.addDownloadedFileToExtension(
            extension,
            result.localPath,
            resource.url,
          );
  ...
  private downloadSingleResource(
    extension: Extension,
  ...
  private downloadResourceSync(
    ...
    try {
      ...
      logger.debug(
        extension,
        `Downloaded: ${resource.url} -> ${resource.localPath}`,
      );
      ...
      return {
        ...
        contentType:
          downloadedData.contentType || this.inferContentType(resource.url),
  ...
  private updateReferencesToLocal(
    ...
    // Update manifest
    extension.manifest = this.replaceUrlsInObject(
      extension.manifest,
      urlMapping,
    );
    ...
    extension.files.forEach((file) => {
      if (
        ...
        try {
          ...
          const updatedContent = this.replaceUrlsInContent(
            originalContent,
            urlMapping,
          );
          ...
          if (originalContent !== updatedContent) {
            logger.debug(
              extension,
              `Updated resource references in: ${file.path}`,
            );
          ...
          logger.warn(
            extension,
            `Failed to update references in file: ${file.path}`,
            { error },
          );
        }
      }

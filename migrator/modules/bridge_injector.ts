...
export class BridgeInjector implements MigrationModule {
    ...
    private static loadBridgeContent(): string {
        try {
            ...
            throw new Error(`Failed to load bridge file: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    private static createBridgeFile(): LazyFile {
        ...
        bridgeFile.close = () => { /* No-op for in-memory content */ };
        bridgeFile.getAST = () => {
    ...
    private static injectBridgeIntoManifest(manifest: any): any {
        ...
        if (updatedManifest.background && updatedManifest.background.service_worker) {
            ...
            logger.debug(null, "Service worker detected, bridge injection may need additional handling", {
        ...
        if (updatedManifest.content_scripts && updatedManifest.content_scripts.length > 0) {
            ...
            if (updatedManifest.manifest_version === 3) {
                const existingResource = updatedManifest.web_accessible_resources.find(
                    (resource: any) => resource.resources && resource.resources.includes(BridgeInjector.BRIDGE_FILENAME)
                );
                ...
                if (!updatedManifest.web_accessible_resources.includes(BridgeInjector.BRIDGE_FILENAME)) {
    ...
    public static migrate(extension: Extension): Extension | MigrationError {
        ...
        try {
            ...
            const hasBridge = extension.files.some(file => file.path === BridgeInjector.BRIDGE_FILENAME);
            if (hasBridge) {
    ...
    public static hasBridgeInManifest(manifest: any): boolean {
        ...
        if (manifest.content_scripts && Array.isArray(manifest.content_scripts)) {
            return manifest.content_scripts.some((contentScript: any) =>

...
export class InterestingnessScorer implements MigrationModule {
  ...
  public static migrate(extension: Extension): Extension | MigrationError {
    try {
      const score =
        InterestingnessScorer.calculateInterestingnessScore(extension);
      ...
      logger.debug(
        extension,
        `Calculated interestingness score: ${score.total}`,
        {
    ...
    for (const file of extension.files) {
      try {
        ...
        if (file.filetype === ExtFileType.JS) {
          // webRequest patterns
          webRequestCount += InterestingnessScorer.countPattern(
            content,
            /webRequest/g,
          );
  ...
  private static analyzeManifest(
    extension: Extension,
    scores: InterestingnessBreakdown,
  ): void {
  ...
  private static calculateExtensionSize(
    ...
    for (const file of extension.files) {
      try {
        ...
        logger.debug(
          extension,
          `Error calculating size for file ${file.path}`,
          { error },
        );
      }
    }
  ...
  private static analyzeMigrationChanges(
    ...
    if (extension.manifest) {
      ...
      if (
        ...
        extension.mv3_extension_id !== extension.id
      ) {

...
export class RenameAPIS implements MigrationModule {
  ...
  public static migrate(extension: Extension): Extension | MigrationError {
    ...
    try {
      ...
      if (mappings.mappings.length === 0) {
        return new MigrationError(
          extension,
          new Error("No API mappings available"),
        );
      }
  ...
  private static processJavaScriptFile(
      ...
      if (isLargeFile && potentialTransformations > 0) {
        ...
        logger.error(
          ...
          "AST parsing failed for large file with API transformations needed",
          {
            ...
            issue:
              "Large files (>100KB) cannot be parsed for API transformations",
          },
        );
        ...
        logger.error(
          null,
          "AST parsing failed for file with API transformations needed",
          {
            path: file.path,
            fileSizeKB: fileSizeKB,
            potentialTransformations: potentialTransformations,
        ...
        logger.debug(
          null,
          "AST parsing failed but no API transformations needed",
          {
            path: file.path,
            fileSizeKB: fileSizeKB,
          },
    ...
    const { transformedAST, transformationCount } =
      RenameAPIS.applyApiTransformations(ast, mappings, file.path);
    ...
    const newContent =
      FormatPreservingGenerator.generateWithPreservedFormatting(
        transformedAST,
        originalContent,
      );
    onTransformed(true);
    ...
    RenameAPIS.traverseAST(transformedAST, (node: any) => {
      ...
      for (const mapping of mappings.mappings) {
        if (RenameAPIS.nodeMatchesSourcePattern(node, mapping.source)) {
          RenameAPIS.applyTargetTransformation(
            node,
            mapping.target,
            mapping.source,
          );
  ...
  private static nodeMatchesSourcePattern(node: any, source: any): boolean {
    ...
    const sourcePattern = source.body
      .replace(/^return\s+/, "")
      .replace(/;$/, "");
    ...
    if (
      ...
      return (
        sourcePattern === apiPath || sourcePattern.startsWith(apiPath + "(")
      );
    }
   ...
   */
  private static applyTargetTransformation(
    node: any,
    target: any,
    source?: any,
  ): void {
    const targetPattern = target.body
      .replace(/^return\s+/, "")
      .replace(/;$/, "");
    ...
    if (
      ...
      if (
        source &&
        RenameAPIS.isParameterTransformationRequired(source, target)
      ) {
   ...
   */
  private static isParameterTransformationRequired(
    source: any,
    target: any,
  ): boolean {
  ...
  private static transformExecuteScriptParameters(callNode: any): void {
    ...
    if (
      ...
      const injectionObject = RenameAPIS.createInjectionObject(
        tabIdArg,
        detailsArg,
      );
  ...
  private static createInjectionObject(tabIdArg: any, detailsArg: any): any {
    ...
    if (
      detailsArg &&
      detailsArg.type === "ObjectExpression" &&
      detailsArg.properties
    ) {
   ...
   */
  private static updateMemberExpressionPath(
    memberExpr: any,
   ...
   */
  private static createTransformedFile(
    originalFile: LazyFile,
    newContent: string,
  ): LazyFile {
  ...
  private static countPotentialTransformations(
    ...
    for (const mapping of mappings.mappings) {
      ...
      const sourcePattern = mapping.source.body
        .replace(/^return\s+/, "")
        .replace(/;$/, "");

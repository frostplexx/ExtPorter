...
4. **Start the server and database**
    ...
    # Or manually set user ID environment variables
    ...
    **Note**: Using `./scripts/docker-compose.sh` or setting USER_ID/GROUP_ID ensures that all files generated by the migrator (in the `output/`, `logs/`, and `cws/` directories) are owned by your current user instead of root.

...
import {
  Database,
  Collections,
} from "../migrator/features/database/db_manager";
...
class ExtensionSearcher {
  ...
  formatExtensionForFzf(ext: Extension): string {
    ...
    const mv3Id = ext.mv3_extension_id
  ...
  displayExtensionDetails(ext: Extension) {
    ...
    console.log(
      `Description: ${ext.manifest?.description || "No description"}`,
    );
    ...
    if (ext.interestingness_score !== undefined) {
      ...
      if (ext.interestingness_breakdown) {
        console.log("\nScore Breakdown:");
        Object.entries(ext.interestingness_breakdown).forEach(
          ([key, value]) => {
            if (value > 0) {
    ...
    if (ext.manifest) {
      ...
      Object.keys(ext.manifest).forEach((key) => {
        if (
          !["name", "description", "version", "manifest_version"].includes(key)
        ) {
          console.log(`  - ${key}`);
        }
      });

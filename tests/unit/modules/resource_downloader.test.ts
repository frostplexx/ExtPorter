...
describe('ResourceDownloader', () => {
  const testDir = path.join(process.env.TEST_OUTPUT_DIR!, 'resource_downloader_test');
  ...
  function createTestExtension(name: string, manifest: any, files: Array<{name: string, content: string, type: ExtFileType}>): Extension {
  ...
  describe('migrate', () => {
    it('should return extension unchanged when no remote resources found', () => {
      const extension = createTestExtension('no-remote', {
        name: 'No Remote Extension',
        version: '1.0',
        manifest_version: 2
      }, [
        { name: 'content.js', content: 'console.log("local");', type: ExtFileType.JS },
        { name: 'style.css', content: 'body { color: red; }', type: ExtFileType.CSS }
      ]);
    ...
    it('should find and process remote resources in JavaScript files', () => {
      const extension = createTestExtension('js-remote', {
    ...
    it('should find and process remote resources in CSS files', () => {
      const extension = createTestExtension('css-remote', {
    ...
    it('should find and process remote resources in HTML files', () => {
      const extension = createTestExtension('html-remote', {
    ...
    it('should find remote resources in manifest', () => {
      const extension = createTestExtension('manifest-remote', {
    ...
    it('should handle multiple remote resources correctly', () => {
      const extension = createTestExtension('multiple-remote', {
      ...
      if (!(result instanceof MigrationError)) {
        ...
        const downloadedFiles = result.files.filter(f => f.path.startsWith('remote_resources/'));
    ...
    it('should filter out invalid URLs', () => {
      const extension = createTestExtension('invalid-urls', {
    ...
    it('should create organized directory structure for downloads', () => {
      const extension = createTestExtension('organized-downloads', {
    ...
    it('should handle extensions with no files', () => {
      const extension = createTestExtension('no-files', {
  ...
  describe('URL pattern matching', () => {
    it('should match Google Fonts URLs', () => {
      const extension = createTestExtension('google-fonts', {
    ...
    it('should match CDN URLs', () => {
      const extension = createTestExtension('cdn-test', {
        name: 'CDN Test',
        version: '1.0',
        manifest_version: 2
      }, [

  ...
  describe('Content Security Policy migration', () => {
    describe('MV2 string format to MV3 object format', () => {
      it('should convert MV2 CSP string to MV3 object format', () => {
        baseExtension.manifest.content_security_policy = "script-src 'self'; object-src 'self';";
      ...
      it('should validate and sanitize insecure MV2 CSP', () => {
        baseExtension.manifest.content_security_policy = "script-src 'self' 'unsafe-eval'; object-src 'self';";

        const result = MigrateManifest.migrate(baseExtension);

        expect(result).not.toBeInstanceOf(MigrationError);
        if (!(result instanceof MigrationError)) {
          expect(result.manifest.content_security_policy.extension_pages).not.toContain('unsafe-eval');
          expect(result.manifest.content_security_policy.extension_pages).toContain("script-src 'self'");
        }
      });
    });
    ...
    describe('Hash directive sanitization', () => {
      it('should remove the specific insecure SHA256 hash that causes Chrome errors', () => {
        const insecureCSP = "script-src 'self' 'sha256-iZBJenro+ON4QTZuWnyvHk3Yj9s/TfHgJLTCP8EJzhE='; object-src 'self';";
        ...
        expect(result).not.toBeInstanceOf(MigrationError);
        if (!(result instanceof MigrationError)) {
          expect(result.manifest.content_security_policy.extension_pages)
            .not.toContain('sha256-iZBJenro+ON4QTZuWnyvHk3Yj9s/TfHgJLTCP8EJzhE=');
          expect(result.manifest.content_security_policy.extension_pages)
      ...
      it('should remove all SHA-based hashes from CSP', () => {
        const hashCSP = "script-src 'self' 'sha256-abc123==' 'sha384-def456==' 'sha512-ghi789=='; object-src 'self';";
    ...
    describe('MV3 object format validation', () => {
      ...
      it('should add missing extension_pages to existing MV3 format', () => {
        ...
        if (!(result instanceof MigrationError)) {
          ...
          expect(result.manifest.content_security_policy.sandbox)
            .toBe("allow-scripts");
        }
      });
    });
    ...
    describe('Complex CSP scenarios', () => {
      it('should handle multiple problematic directives in one CSP', () => {
        const complexCSP = "script-src 'self' 'unsafe-eval' 'unsafe-inline' 'sha256-abc123==' data: http://bad.com 'nonce-xyz'; style-src 'self' 'unsafe-inline'; object-src 'self';";
      ...
      it('should handle real-world problematic CSP from Chrome Web Store extensions', () => {
        ...
        const realWorldCSP = "script-src 'self' 'wasm-unsafe-eval' 'sha256-iZBJenro+ON4QTZuWnyvHk3Yj9s/TfHgJLTCP8EJzhE='; object-src 'self';";
      ...
      it('should remove bare JavaScript file paths that cause Chrome MV3 errors', () => {
        ...
        const filePathCSP = "script-src 'self' remote_resources/f3d11240_ga.js; object-src 'self';";
      ...
      it('should remove various bare file paths from script-src', () => {
        const multipleFilePathsCSP = "script-src 'self' content.js background.js libs/jquery.min.js vendor/analytics.js; object-src 'self';";
    ...
    describe('Generic pattern detection and removal', () => {
      it('should remove any SHA hash variant (256, 384, 512)', () => {
        const multiHashCSP = "script-src 'self' 'sha256-randomhash1==' 'sha384-anotherhash2==' 'sha512-thirdhash3=='; object-src 'self';";
      ...
      it('should remove any nonce directive regardless of value', () => {
        const nonceCSP = "script-src 'self' 'nonce-abc123' 'nonce-xyz789' 'nonce-random456'; object-src 'self';";
        baseExtension.manifest.content_security_policy = nonceCSP;
      ...
      it('should handle generic JavaScript file patterns in any directory structure', () => {
        const complexFilePathsCSP = "script-src 'self' app.js src/main.js lib/vendor/analytics.js assets/js/tracking.js components/ui/modal.js; object-src 'self';";
      ...
      it('should remove any non-localhost HTTP URLs while preserving localhost', () => {
        const mixedHTTPCSP = "script-src 'self' http://example.com/script.js http://api.service.com/data http://localhost:3000 http://cdn.provider.net/lib.js; object-src 'self';";
      ...
      it('should handle any combination of unsafe directives', () => {
        const unsafeComboCSP = "script-src 'self' 'unsafe-eval' 'unsafe-inline' data: blob:; style-src 'self' 'unsafe-inline'; object-src 'self';";
      ...
      it('should preserve valid HTTPS sources while removing invalid patterns', () => {
        const mixedValidInvalidCSP = "script-src 'self' https://cdn.jsdelivr.net/npm/jquery@3.6.0/dist/jquery.min.js 'sha256-invalid==' badfile.js 'unsafe-eval' https://apis.google.com/js/platform.js data:; object-src 'self';";
        ...
        if (!(result instanceof MigrationError)) {
          ...
          expect(csp).toContain('https://cdn.jsdelivr.net/npm/jquery@3.6.0/dist/jquery.min.js');
      ...
      it('should handle edge cases in file path detection', () => {
        const edgeCaseCSP = "script-src 'self' file.js path/to/file.js deep/nested/path/to/script.js single.js; object-src 'self';";
      ...
      it('should be extensible for future MV3 violations', () => {
        ...
        const futureProblemCSP = "script-src 'self' 'sha1-oldhash==' 'md5-evenworsehash=='; object-src 'self';";

...
`;
      ...
      const outputComments = (result.match(/\/\/.*|\/\*[\s\S]*?\*\//g) || [])
        .length;
...
`;
      ...
      const inputComments = (sourceCode.match(/\/\/.*|\/\*[\s\S]*?\*\//g) || [])
        .length;
      const outputComments = (result.match(/\/\/.*|\/\*[\s\S]*?\*\//g) || [])
        .length;
      ...
      const inputCommentsCount = (
        sourceCode.match(/\/\/.*|\/\*[\s\S]*?\*\//g) || []
      ).length;
      expect(outputComments).toBeGreaterThanOrEqual(
        Math.floor(inputCommentsCount * 0.7),
      ); // At least 70% preserved
    });
...
`;
      ...
      const totalInputComments = (
        sourceCode.match(/\/\/.*|\/\*[\s\S]*?\*\//g) || []
      ).length;
      const totalOutputComments = (
        result.match(/\/\/.*|\/\*[\s\S]*?\*\//g) || []
      ).length;

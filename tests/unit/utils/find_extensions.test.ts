...
describe("find_extensions", () => {
  ...
  describe("file type detection", () => {
    it("should correctly categorize different file types", () => {
      ...
      fs.writeFileSync(
        path.join(extensionDir, "script.js"),
        'console.log("js");',
      );
      fs.writeFileSync(
        path.join(extensionDir, "style.css"),
      ...
      fs.writeFileSync(
        path.join(extensionDir, "data.json"),
        '{"key": "value"}',
      );
      ...
      const jsFiles = extension.files.filter(
        (f) => f.filetype === ExtFileType.JS,
      );
      const cssFiles = extension.files.filter(
        (f) => f.filetype === ExtFileType.CSS,
      );
      const htmlFiles = extension.files.filter(
        (f) => f.filetype === ExtFileType.HTML,
      );
      const otherFiles = extension.files.filter(
        (f) => f.filetype === ExtFileType.OTHER,
      );
    ...
    it("should skip very small files", () => {
      ...
      // Create normal file
      fs.writeFileSync(
        path.join(extensionDir, "normal.js"),
        'console.log("test");',
      );

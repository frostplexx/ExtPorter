...
describe("Mock Extensions Puppeteer Tests", () => {
  ...
  const popupExtensionPath = path.join(
    __dirname,
    "../fixtures/mock-extensions/popup-extension",
  );
  ...
  describe("New Tab Extension Tests", () => {
    test("should override new tab page with custom dashboard", async () => {
      ...
      const weatherWidget = await newTabPage.$(
        '[data-testid="weather-widget"]',
      );
      const bookmarksWidget = await newTabPage.$(
    ...
    test("should test bookmark functionality integration", async () => {
      ...
      const bookmarksList = await newTabPage.$(

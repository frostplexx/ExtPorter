...
async function initializeNewTab() {
  ...
  document.body.setAttribute(
    "data-load-time",
    newTabState.startTime.toString(),
  );
...
function initializeWeather() {
  ...
  elements.weatherCondition.setAttribute(
    "data-condition",
    mockWeatherData.condition,
  );

  elements.humidity.textContent = `${mockWeatherData.humidity}%`;
  elements.humidity.setAttribute(
    "data-humidity",
    mockWeatherData.humidity.toString(),
  );
...
async function initializeBookmarks() {
  ...
  try {
    ...
    elements.bookmarksList.setAttribute(
      "data-bookmark-count",
      bookmarks.length.toString(),
    );
    newTabState.widgetsLoaded.bookmarks = true;
    console.log(
      `✅ Bookmarks widget initialized with ${bookmarks.length} bookmarks`,
    );
  } catch (error) {
...
async function initializeTopSites() {
  ...
  try {
    ...
    if (topSites.length === 0) {
      elements.topsitesList.innerHTML =
        '<div class="empty-state">No top sites found</div>';
    } else {
      // Display top sites
    ...
    elements.topsitesList.setAttribute(
      "data-topsite-count",
      topSites.length.toString(),
    );
    newTabState.widgetsLoaded.topSites = true;
    console.log(
      `✅ Top sites widget initialized with ${topSites.length} sites`,
    );
  } catch (error) {
    console.error("❌ Failed to load top sites:", error);
    elements.topsitesList.innerHTML =
      '<div class="error">Failed to load top sites</div>';
...
function initializeRecentActivity() {
  ...
  elements.recentList.setAttribute(
    "data-recent-count",
    mockRecentActivity.length.toString(),
  );
...
function setupEventListeners() {
  ...
  elements.weatherToggle.addEventListener("click", () =>
    toggleSetting("showWeather"),
  );
  elements.bookmarksToggle.addEventListener("click", () =>
    toggleSetting("showBookmarks"),
  );
  elements.topsitesToggle.addEventListener("click", () =>
    toggleSetting("showTopSites"),
  );
  elements.timeFormatToggle.addEventListener("click", () =>
    toggleSetting("use24HourFormat"),
  );

  // Quick action buttons
  elements.newTabBtn.addEventListener("click", () => chrome.tabs.create({}));
  elements.newWindowBtn.addEventListener("click", () =>
  ...
  elements.testBookmarksBtn.addEventListener(
    "click",
    testBookmarksFunctionality,
  );
...
function toggleSetting(settingName) {
  ...
  if (toggleElement) {
    ...
    toggleElement.setAttribute(
      "data-active",
      newTabState.settings[settingName].toString(),
    );
  }
  ...
  console.log(
    `⚙️ Setting ${settingName} changed to:`,
    newTabState.settings[settingName],
  );
}
...
function applySettingChange(settingName) {
  switch (settingName) {
    case "showWeather":
      ...
      weatherWidget.style.display = newTabState.settings.showWeather
        ? "block"
        : "none";
      break;

    case "showBookmarks":
      const bookmarksWidget = document.querySelector(
        '[data-testid="bookmarks-widget"]',
      );
      bookmarksWidget.style.display = newTabState.settings.showBookmarks
        ? "block"
        : "none";
      break;

    case "showTopSites":
      const topsitesWidget = document.querySelector(
        '[data-testid="topsites-widget"]',
      );
      topsitesWidget.style.display = newTabState.settings.showTopSites
        ? "block"
        : "none";
      break;
...
async function testBookmarksFunctionality() {
  ...
  try {
    ...
    elements.testBookmarksBtn.setAttribute(
      "data-bookmark-count",
      bookmarks.length.toString(),
    );
...
async function loadSettings() {
  try {
    ...
    elements.weatherToggle.classList.toggle(
      "active",
      newTabState.settings.showWeather,
    );
    elements.bookmarksToggle.classList.toggle(
      "active",
      newTabState.settings.showBookmarks,
    );
    elements.topsitesToggle.classList.toggle(
      "active",
      newTabState.settings.showTopSites,
    );
    elements.timeFormatToggle.classList.toggle(
      "active",
      newTabState.settings.use24HourFormat,
    );

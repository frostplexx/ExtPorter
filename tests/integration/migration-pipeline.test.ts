...
describe('Migration Pipeline Integration Tests', () => {
  ...
  describe('Migration Pipeline Consistency', () => {
    it('should produce consistent results when run multiple times', () => {
      ...
      const extension2 = createTestExtension(SAMPLE_EXTENSIONS[1], path.join(testDir, 'second'));

      // Run pipeline on both extensions
      const result1 = ResourceDownloader.migrate(MigrateManifest.migrate(extension1) as Extension);
      const result2 = ResourceDownloader.migrate(MigrateManifest.migrate(extension2) as Extension);
      ...
      if (!(result1 instanceof MigrationError) && !(result2 instanceof MigrationError)) {
        ...
        expect(result1.manifest.host_permissions).toEqual(result2.manifest.host_permissions);

        // Both should have downloaded the same number of remote resources
        const result1Downloads = result1.files.filter(f => f.path.startsWith('remote_resources/')).length;
        const result2Downloads = result2.files.filter(f => f.path.startsWith('remote_resources/')).length;
        expect(result1Downloads).toBe(result2Downloads);
      }

      [extension1, extension2].forEach(ext => ext.files.forEach((file: { close: () => any; }) => file.close()));
      ...
      SAMPLE_EXTENSIONS.forEach(fixture => {
        ...
        if (!(result instanceof MigrationError)) {
          ...
          if (fixture.manifest.content_scripts) {
            expect(result.manifest.content_scripts).toEqual(fixture.manifest.content_scripts);
          }
          ...
          if (fixture.manifest.chrome_url_overrides) {
            expect(result.manifest.chrome_url_overrides).toEqual(fixture.manifest.chrome_url_overrides);
          }
        }

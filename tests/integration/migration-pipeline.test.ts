...
import {
  SAMPLE_EXTENSIONS,
  createTestExtension,
} from "../fixtures/sample-extensions";
...
describe("Migration Pipeline Integration Tests", () => {
  ...
  describe("Simple Extension Migration", () => {
    it("should successfully migrate a simple extension through the complete pipeline", () => {
      ...
      if (!(manifestResult instanceof MigrationError)) {
        ...
        expect(manifestResult.manifest.background.service_worker).toBe(
          "background.js",
        );
  ...
  describe("Complex Extension Migration", () => {
    it("should successfully migrate a complex extension with remote resources", () => {
      ...
      if (!(manifestResult instanceof MigrationError)) {
        ...
        expect(manifestResult.manifest.manifest_version).toBe(3);
        expect(manifestResult.manifest.permissions).toContain(
          "declarativeNetRequest",
        );
        expect(manifestResult.manifest.permissions).not.toContain(
          "webRequestBlocking",
        );
        expect(manifestResult.manifest.host_permissions).toContain(
          "http://example.com/*",
        );
        ...
        if (!(resourceResult instanceof MigrationError)) {
          ...
          expect(resourceResult.files.length).toBeGreaterThan(
            extension.files.length,
          );
          ...
          const contentFile = resourceResult.files.find(
            (f) => f.path === "content.js",
          );
          ...
          const cssFile = resourceResult.files.find(
            (f) => f.path === "content.css",
          );
  ...
  describe("New Tab Extension Migration", () => {
    it("should correctly identify and migrate new tab extensions", () => {
      ...
      if (!(manifestResult instanceof MigrationError)) {
        ...
        expect(manifestResult.manifest.chrome_url_overrides.newtab).toBe(
          "newtab.html",
        );
        ...
        if (!(resourceResult instanceof MigrationError)) {
          const newtabFile = resourceResult.files.find(
            (f) => f.path === "newtab.html",
          );
  ...
  describe("Migration Error Handling", () => {
    ...
    it("should handle extensions with missing files gracefully", () => {
      const extensionWithMissingFiles = createTestExtension(
        SAMPLE_EXTENSIONS[0],
        testDir,
      );
  ...
  describe("Migration Pipeline Consistency", () => {
    it("should produce consistent results when run multiple times", () => {
      ...
      if (
        !(result1 instanceof MigrationError) &&
        !(result2 instanceof MigrationError)
      ) {
        // Both should have the same manifest structure
        expect(result1.manifest.manifest_version).toBe(
          result2.manifest.manifest_version,
        );
        expect(result1.manifest.permissions).toEqual(
          result2.manifest.permissions,
        );
      ...
      if (!(result1 instanceof MigrationError))
        result1.files.forEach((file) => file.close());
      if (!(result2 instanceof MigrationError))
        result2.files.forEach((file) => file.close());
    });
  });
      ...
      SAMPLE_EXTENSIONS.forEach((fixture) => {
        ...
        if (!(result instanceof MigrationError)) {
          ...
          if (fixture.manifest.options_page) {
            expect(result.manifest.options_page).toBe(
              fixture.manifest.options_page,
            );
          }

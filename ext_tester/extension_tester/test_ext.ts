...
async function testPopUpFunctionality(extension: Extension, browser: Browser): Promise<TestResult | null> {
    ...
    if (popupPath) {
        ...
        try {
            ...
            // FIXME: This gets stuck
            // Test basic interactions - actually click on interactive elements
            // const clickableSelectors = ['button', 'a[href]', 'input[type="button"]', 'input[type="submit"]', '[role="button"]'];
            // let successfulClicks = 0;
            //
            // for (const selector of clickableSelectors) {
            //     try {
            //         const elements = await page.$$(selector);
            //         for (let i = 0; i < Math.min(elements.length, 3); i++) { // Limit to 3 clicks per selector type
            //             try {
            //                 await elements[i].click();
            //                 successfulClicks++;
            //             } catch (clickError) {
            //                 console.warn(`Failed to click ${selector} element ${i}:`, clickError);
            //             }
            //         }
            //     } catch (error) {
            //         // Selector not found or other error, continue
            //     }
            // }
